...
import { SubjectDetailFilter } from "../Helper/SubjectDetailFilter";

const ScoreTeacher = ({ reload }) => {
  ...
  const getStudents = async (subjectDetailId) => {
    if (!subjectDetailId) return;
    try {
      const response = await fetch(`${SERVER_URL}/get-score-by-teacher`, {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    if (selectedSubjectDetail) {
      getStudents(selectedSubjectDetail);
    }
  }, [selectedSubjectDetail, reload]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      getStudents(selectedSubjectDetail); // Load lại dữ liệu sau khi cập nhật
    } catch (error) {
  ...
  return (
    <div>
      ...
      <div>
        ...
        <SubjectDetailFilter
          selectedSubjectDetail={selectedSubjectDetail}
